...
for dir in GSE*/; do
    ...
    # 只处理包含 paired_Ecoli.sh 的文件夹
    if [ ! -f "$dir/paired_Ecoli.sh" ]; then
        echo "Skipping $dir (no paired_Ecoli.sh found)"
        continue
    fi
    ...
    if [ "$run_script" = true ]; then
        ...
        # 执行 paired_Ecoli.sh
        echo "Executing $dir/paired_Ecoli.sh"
        bash "paired_Ecoli.sh"
        echo "$dir/paired_Ecoli.sh completed."
